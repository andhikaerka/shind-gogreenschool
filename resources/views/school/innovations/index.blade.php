...
@section('title')
    {{ trans('global.list') }} {{ trans('crud.innovation.title_singular') }} - {{ trans('panel.site_title') }}
@endsection

@section('content')
    @if($school_slug == auth()->user()->school_slug)
        @can('innovation_create')
            <div style="margin-bottom: 10px;" class="row">
                <div class="col-lg-12">
                    <a class="btn btn-success"
                       href="{{ route("school.innovations.create", ['school_slug' => $school_slug, 'year' => request()->input('year')]) }}">
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.list') }} {{ trans('crud.innovation.title_singular') }}
        </div>
        ...
        <div class="card-body">
            <form action="" method="get" class="mb-3">
                <div class="row form-row">
                    ...
                    <div class="col mr-sm-2">
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text bg-transparent border-0">Kondisi {{ trans('crud.innovation.title') }}&nbsp;:</span>
                            </div>
                            <div class="input-group-append">
            ...
            <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Innovation">
                <thead>
                <tr>
                    <th style="width: 10px;">
                    ...
                    <th>
                        {{ trans('crud.innovation.fields.name') }}
                    </th>
                    <th>
                        {{ trans('crud.innovation.fields.aspect') }}
                    </th>
                    <th>
                    ...
                    <th>
                        {{ trans('crud.innovation.fields.tutor') }}
                    </th>
                    <th>
                        {{ trans('crud.innovation.fields.purpose') }}
                    </th>
                    <th>
                        {{ trans('crud.innovation.fields.team_statuses') }}
                    </th>
                    <th>
                        {{ trans('crud.innovation.fields.advantage') }}
                    </th>
                    <th>
                        {{ trans('crud.innovation.fields.innovation') }}
                    </th>
                    <th>
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            @if($school_slug == auth()->user()->school_slug)
            @can('innovation_delete')
            let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
            let deleteButton = {
                text: deleteButtonTrans,
                url: "{{ route('school.innovations.massDestroy', ['school_slug' => $school_slug]) }}",
            ...
            let dtOverrideGlobals = {
                ...
                aaSorting: [],
                ajax: {
                    url: "{{ route('school.innovations.index', ['school_slug' => $school_slug]) }}",
                    data: {
            ...
            $('.datatable-Innovation').DataTable(dtOverrideGlobals);
