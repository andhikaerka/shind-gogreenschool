...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST"
                  action="{{ route("school.cadres.update", ['school_slug' => $school_slug, $cadre->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('class') ? 'is-invalid' : '' }}" name="class" id="class"
                        ...
                        @foreach($classes as $key => $class)
                            <option
                                value="{{ $class['id'] }}" {{ old('class', $cadre->class) == $class['id'] ? 'selected' : '' }}>
                                {{ $class['text'] }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('address') ? 'is-invalid' : '' }}" name="address"
                              ...
                              required>{{ old('address', $cadre->address) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('hobby') ? 'is-invalid' : '' }}" name="hobby"
                              id="hobby" maxlength="{{ \App\Cadre::MAX_LENGTH_OF_HOBBY }}"
                              required>{{ old('hobby', $cadre->hobby) }}</textarea>
                    @if($errors->has('hobby'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2-tags {{ $errors->has('position') ? 'is-invalid' : '' }}"
                        ...
                        @endforeach
                        @if(!in_array(old('position', $cadre->position), App\Cadre::POSITION_SELECT))
                            <option
                                value="{{ $cadre->position }}" selected>{{ $cadre->position }}</option>
                        @endif
                    </select>
                ...
                <div class="form-group">
                    <label for="password-confirm">{{ __('global.confirm_password') }}</label>
    ...
    <script>
        ...
        $(function () {
            ...
            axios.post('{{ route('api.work-groups') }}', {
                ...
                let selectedWorkGroupVal = "{!! old('work_group_id', $cadre->work_group_id) !!}";
