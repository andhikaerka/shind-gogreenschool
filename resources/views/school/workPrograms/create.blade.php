...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("school.work-programs.store", ['school_slug' => $school_slug]) }}"
                  enctype="multipart/form-data"
                  id="formCreate">
                @csrf

                {{-- <div class="form-group">
                    <label class="required" for="name">{{ trans('crud.workProgram.fields.name') }}</label>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}" name="condition"
                              id="condition"
                              required>{{ old('condition') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('time') ? 'is-invalid' : '' }}" name="time"
                            id="time" required>
                        <option value
                                disabled {{ old('time', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                        @for($i = \App\WorkProgram::MIN_TIME; $i <= \App\WorkProgram::MAX_TIME; $i++)
                            <option
                                value="{{ $i }}" {{ old('time', '') == $i ? 'selected' : '' }}>{{ $i }} Bulan
                            </option>
                        @endfor
                    </select>
                    @if($errors->has('time'))
                ...
                <div class="form-group">
                    <div class="form-group">
                        ...
                        {{-- <select class="form-control select2 {{ $errors->has('tutor_1') ? 'is-invalid' : '' }}"
                            ...
                            {{ app()->environment() == 'production' ? 'required' : '' }}>
                            @foreach($tutors_1 as $id => $tutor)
                                <option
                                    value="{{ $id }}" {{ old('tutor_1', '') == $id ? 'selected' : '' }}>{{ $tutor }}</option>
                            @endforeach
                        </select> --}}
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('tutor_2') ? 'is-invalid' : '' }}"
                            ...
                            {{ app()->environment() == 'production' ? 'required' : '' }}>
                            @foreach($tutors_2 as $id => $tutor)
                                <option
                                    value="{{ $id }}" {{ old('tutor_2', '') == $id ? 'selected' : '' }}>{{ $tutor }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <label class="required"
                               for="tutor_3">{{ trans('crud.workProgram.fields.tutor_3') }}</label>
                        ...
                        @endif
                        <span class="help-block">{{ trans('crud.workProgram.fields.tutor_3_helper') }}</span>
                    </div>
                </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                        ...
                        <label class="required form-check-label font-weight-bold"
                ...
                <div class="form-group">
                    ...
                    <a class="btn btn-warning"
                       href="{{ route("school.work-programs.index", ['school_slug' => $school_slug]) }}">
                        Tidak Keluar
                    </a>
                </div>
            </form>
...
@section('scripts')
    ...
    <script>
        ...
        $(function () {
            ...
            selectAspectId.val("{!! old('aspect_id', '0') !!}").trigger('change');
                ...
                if (selectWorkGroupId.val()) {
                    ...
                    axios.post('{{ route('api.studies') }}', {
                        ...
                        .then(function (response) {
                            ...
                            if (studyIdVal) {
                                selectStudyId.val(studyIdVal).trigger('change');
                            }
                        })
                    ...
                    axios.post('{{ route('api.teams') }}', {
                        ...
                        .then(function (response) {
                            ...
                            if (tutor3Val) {
                                tutor3.val(tutor3Val).trigger('change');
                            }
                        })
                    ...
                    .then(function (response) {
                        ...
                        if(response.data.maxPercentage == 0){
                            percentageStatus.text('sudah terpenuhi 100%')
                            percentage.prop( "disabled", true );
                        }else{
                            percentageStatus.text('Sudah terpakai '+(100-response.data.maxPercentage)+'% dan sisa '+response.data.maxPercentage+'%')
        ...
        Dropzone.options.photoDropzone = {
            ...
            success: function (file, response) {
                $('form#formCreate').find('input[name="photo"]').remove();
                $('form#formCreate').append('<input type="hidden" name="photo" value="' + response.name + '">')
            },
            ...
            init: function () {
                ...
                $('form#formCreate').append('<input type="hidden" name="photo" value="' + file.file_name + '">');
                this.options.maxFiles = this.options.maxFiles - 1;
                @endif
            },
