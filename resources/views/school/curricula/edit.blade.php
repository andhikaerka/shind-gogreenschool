...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('crud.curriculum.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("school.curricula.update", ['school_slug' => $school_slug, $curriculum->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('vision') ? 'is-invalid' : '' }}" name="vision"
                              id="vision" required>{{ old('vision', $curriculum->vision) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('mission') ? 'is-invalid' : '' }}" name="mission"
                              id="mission" required>{{ old('mission', $curriculum->mission) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('purpose') ? 'is-invalid' : '' }}" name="purpose"
                              id="purpose" required>{{ old('purpose', $curriculum->purpose) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('calendars') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($calendars as $id => $calendar)
                            <option
                                value="{{ $id }}" {{ (in_array($id, old('calendars', [])) || $curriculum->calendars->contains($id)) ? 'selected' : '' }}>{{ $calendar }}</option>
                        @endforeach
                    </select>
