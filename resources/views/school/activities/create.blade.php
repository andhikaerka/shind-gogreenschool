...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("school.activities.store", ['school_slug' => $school_slug]) }}"
                  id="formCreate">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('advantage') ? 'is-invalid' : '' }}" name="advantage"
                              id="advantage" required>{{ old('advantage') }}</textarea>
                ...
                <div class="form-group">
                    <label class="required"
                           for="document">{{ trans('crud.activity.fields.document') }}</label>
                ...
                if(selectWorkGroupId.val()){
                    axios.post('{{ route('api.teams') }}', {
                        ...
                        .then(function (response) {
                            ...
                            if (tutorVal) {
                                tutor.val(Number('{{ old('tutor', '0') }}')).trigger('change');
                            }
                        })
                }
            });
