...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('crud.team.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("school.teams.update", ['school_slug' => $school_slug, $team->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('team_status_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($teamStatuses as $id => $teamStatus)
                            <option
                                value="{{ $id }}" {{ old('team_status_id', $team->team_status_id) == $id ? 'selected' : '' }}>{{ $teamStatus }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('gender') ? 'is-invalid' : '' }}" name="gender"
                        ...
                        @foreach(App\Team::GENDER_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('gender', $team->gender) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('birth_date') ? 'is-invalid' : '' }}" type="text"
                           ...
                           name="birth_date" id="birth_date" value="{{ old('birth_date', $team->birth_date) }}"
                           required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('aspect_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($aspects as $id => $aspect)
                            <option
                                value="{{ $id }}" {{ old('aspect_id', $team->work_group->aspect_id) == $id ? 'selected' : '' }}>{{ $aspect }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    >{{ old('job_description', $team->job_description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('team_position_id') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($teamPositions as $id => $teamPosition)
                            <option
                                value="{{ $id }}" {{ old('team_position_id', $team->team_position_id) == $id ? 'selected' : '' }}>{{ $teamPosition }}</option>
                        @endforeach
                    </select>
                ...
                <div id="inputAnotherPosition" class="form-group" style="display: none;">
                    ...
                    <input class="form-control {{ $errors->has('another_position') ? 'is-invalid' : '' }}" type="text"
                           ...
                           id="another_position" value="{{ old('another_position', $team->another_position) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email"
                           name="email" id="email" value="{{ old('email', $team->user->email) }}" required>
                    @if($errors->has('email'))
                ...
                {{--<div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('username') ? 'is-invalid' : '' }}" type="text"
                           name="username" id="username" value="{{ old('username', $team->user->username) }}" required>
        ...
        $(function () {
            ...
            inputTeamPositionId.change(function () {
                if (Number(inputTeamPositionId.val()) === 7) {
                    ...
                    if (Number("{{ old('team_position_id', $team->team_position_id) }}") !== 7) {
                        $('#another_position').val('')
                    }
                }
            });

            inputTeamPositionId.val({{ old('team_position_id', $team->team_position_id) }}).trigger('change');
        });
        ...
        $(function () {
            ...
            selectAspectId.change(function () {
                axios.post('{{ route('api.work-groups') }}', {
                    ...
                    year: {{ $team->work_group->school_profile->year }}
                })
                .then(function (response) {
                    selectWorkGroupId.empty();

                    let workGroupIdVal = "{!! old('work_group_id', $team->work_group_id) !!}";
