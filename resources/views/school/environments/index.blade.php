...
@section('content')
    @if($school_slug == auth()->user()->school_slug)
        @can('quality_report_create')
            <div style="margin-bottom: 10px; display: none;" class="row" id="createEDSLH">
                <div class="col-lg-12">
                    <a class="btn btn-success"
                       href="{{ route("school.environments.create", ['school_slug' => $school_slug, 'year' => request()->input('year')]) }}">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-Environment">
                <thead>
                <tr>
                    <th style="width: 10px;">
                    ...
                    <th>
                        KOMPENTENSI KELULUSAN
                    </th>
                    <th>
                        PENDIDIK DAN TENGAGA KEPENDIDIKAN
                    </th>
                    <th>
                        SARANA PRASARANA
                    </th>
    ...
    <script>
        $(function () {
            ...
            let deleteButton = {
                ...
                url: "{{ route('school.environments.massDestroy', ['school_slug' => $school_slug]) }}",
            ...
            let dtOverrideGlobals = {
                ...
                aaSorting: [],
                ajax: {
                    url: "{{ route('school.environments.index', ['school_slug' => $school_slug]) }}",
                    data: {
                ...
                columns: [
                    ...
                    {data: 'kompetensi_kelulusan', name: 'kompetensi_kelulusan'},
                    {data: 'pendidik_dan_tenaga_kependidikan', name: 'pendidik_dan_tenaga_kependidikan'},
                    {data: 'sarana_prasarana', name: 'sarana_prasaran'},
                    {data: 'pengelolaan', name: 'pengelolaan'},
                    {data: 'pembiayaan', name: 'pembiayaan'},
                    {data: 'penilaian_pendidikan', name: 'penilaian_pendidikan'},
                ...
                initComplete: function (settings, json) {
                    ...
                    if(json.data.length < 1){
                        $('#createEDSLH').attr('style', 'margin-bottom: 10px; display: block');
                    }
                }
            };
            $('.datatable-Environment').DataTable(dtOverrideGlobals);
