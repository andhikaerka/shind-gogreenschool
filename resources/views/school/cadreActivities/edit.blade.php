...
@section('content')
    <div class="card">
        <div class="card-header">
            {{ trans('global.edit') }} {{ trans('crud.cadreActivity.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST"
                  action="{{ route("school.cadre-activities.update", ['school_slug' => $school_slug, $cadreActivity->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}" type="text"
                           ...
                           name="date" id="date" value="{{ old('date', $cadreActivity->date) }}" required>
                    @if($errors->has('date'))
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('work_program') ? 'is-invalid' : '' }}"
                            ...
                            required {{ !(auth()->user()->isSTC ?? false) ? 'disabled' : '' }}>
                        <option value=""
                                disabled {{ old('work_program_id', $cadreActivity->work_program_id) == '' ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}
                        </option>
                        {{--@foreach($workPrograms as $id => $workProgram)
                            <option
                                value="{{ $id }}" {{ old('work_program_id', $cadreActivity->work_program_id) == $id ? 'selected' : '' }}>{{ $workProgram }}</option>
                        @endforeach--}}
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2-tags {{ $errors->has('self_development') ? 'is-invalid' : '' }}" name="self_development"
                        ...
                        @foreach(App\Study::LIST_OF_SELF_DEVELOPMENT as $label)
                            <option
                                value="{{ $label }}" {{ old('self_development', $cadreActivity->self_development) === (string) $label ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}"
                            ...
                            required {{ !(auth()->user()->isSTC ?? false) ? 'disabled' : '' }}>
                        <option value="" disabled {{ old('condition', $cadreActivity->condition) == '' ? 'selected' : '' }}>
                            {{ trans('global.pleaseSelect') }}
                        </option>
                        @foreach(\App\CadreActivity::CONDITION_SELECT as $id => $str)
                            <option
                                value="{{ $id }}" {{ old('condition', $cadreActivity->condition) == $id ? 'selected' : '' }}>{{ $str }}</option>
                        @endforeach
                    </select>
                ...
                {{--<div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('percentage') ? 'is-invalid' : '' }}" type="number"
                           name="percentage"
                           id="percentage" value="{{ old('percentage', $cadreActivity->percentage) }}" step="1"
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('results') ? 'is-invalid' : '' }}" name="results"
                              id="results" maxlength="{{ \App\CadreActivity::MAX_LENGTH_OF_RESULTS }}"
                              required>{{ old('results', $cadreActivity->results) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('problem') ? 'is-invalid' : '' }}" name="problem"
                              id="problem" maxlength="{{ \App\CadreActivity::MAX_LENGTH_OF_PROBLEM }}"
                              required>{{ old('problem', $cadreActivity->problem) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('behavioral') ? 'is-invalid' : '' }}"
                              ...
                              maxlength="{{ \App\CadreActivity::MAX_LENGTH_OF_BEHAVIORAL }}"
                              required>{{ old('behavioral', $cadreActivity->behavioral) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('physical') ? 'is-invalid' : '' }}" name="physical"
                              id="physical" maxlength="{{ \App\CadreActivity::MAX_LENGTH_OF_PHYSICAL }}"
                              required>{{ old('physical', $cadreActivity->physical) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('plan') ? 'is-invalid' : '' }}" name="plan"
                              id="plan" maxlength="{{ \App\CadreActivity::MAX_LENGTH_OF_PLAN }}"
                              required>{{ old('plan', $cadreActivity->plan) }}</textarea>
                    @if($errors->has('plan'))
                ...
                <div class="form-group">
                    <label class="required"
                           for="team_statuses">{{ trans('crud.cadreActivity.fields.team_status') }}</label>
                    ...
                    <select class="form-control select2 {{ $errors->has('team_statuses') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($teamStatuses as $id => $team_status)
                            <option
                                value="{{ $id }}" {{ (in_array($id, old('team_statuses', [])) || $cadreActivity->team_statuses->contains($id)) ? 'selected' : '' }}>{{ $team_status }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('team_statuses'))
                        <span class="text-danger">{{ $errors->first('team_statuses') }}</span>
                    @endif
                    <span class="help-block">{{ trans('crud.cadreActivity.fields.team_status_helper') }}</span>
                </div>
    ...
    <script>
        ...
        $(function () {
            ...
            axios.post('{{ route('api.work-groups') }}', {
                ...
                year: {{ $cadreActivity->work_program->study->work_group->school_profile->year }}
            })
            .then(function (response) {
                selectWorkGroupId.empty();

                let selectWorkGroupIdVal = "{!! old('work_program_id', $cadreActivity->work_program->study->work_group_id) !!}";
                console.log(selectWorkGroupIdVal)
                ...
                if (selectWorkGroupIdVal) {
                    selectWorkGroupId.val(selectWorkGroupIdVal).trigger('change');
                }
            })
                ...
                if (selectWorkGroupId.val()) {
                    ...
                    axios.post('{{ route('api.work-programs') }}', {
                        ...
                        .then(function (response) {
                            ...
                            let workProgramIdVal = Number("{{ old('work_program_id', $cadreActivity->work_program_id) }}");
                            ...
                            if (workProgramIdVal) {
                                selectWorkProgramId.val(('{{ old('work_program_id', $cadreActivity->work_program_id) }}')).trigger('change');
                            }
                        })
                }
            });

            selectWorkGroupId.val(('{{ old('work_group_id', $cadreActivity->work_program->study->work_group_id ?? '') }}')).trigger('change');
