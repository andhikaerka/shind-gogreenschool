    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.lesson-plans.update", [$lessonPlan->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('school') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($schools as $id => $school)
                            <option
                                value="{{ $id }}" {{ ($lessonPlan->school ? $lessonPlan->school->id : old('school_id')) == $id ? 'selected' : '' }}>{{ $school }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <label class="required">{{ trans('crud.lessonPlan.fields.ktsp_or_rpp') }}</label>
                    <select class="form-control {{ $errors->has('ktsp_or_rpp') ? 'is-invalid' : '' }}"
                            name="ktsp_or_rpp" id="ktsp_or_rpp" required>
                        <option value
                        ...
                        @foreach(App\LessonPlan::KTSP_OR_RPP_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('ktsp_or_rpp', $lessonPlan->ktsp_or_rpp) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('vision') ? 'is-invalid' : '' }}" name="vision"
                              id="vision" required>{{ old('vision', $lessonPlan->vision) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('mission') ? 'is-invalid' : '' }}" name="mission"
                              id="mission" required>{{ old('mission', $lessonPlan->mission) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('purpose') ? 'is-invalid' : '' }}" name="purpose"
                              id="purpose" required>{{ old('purpose', $lessonPlan->purpose) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('subject') ? 'is-invalid' : '' }}" type="text"
                           name="subject" id="subject" value="{{ old('subject', $lessonPlan->subject) }}" required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('teacher') ? 'is-invalid' : '' }}" type="text"
                           name="teacher" id="teacher" value="{{ old('teacher', $lessonPlan->teacher) }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('artwork') ? 'is-invalid' : '' }}" name="artwork"
                              id="artwork" required>{{ old('artwork', $lessonPlan->artwork) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('calendars') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($calendars as $id => $calendar)
                            <option
                                value="{{ $id }}" {{ (in_array($id, old('calendars', [])) || $lessonPlan->calendars->contains($id)) ? 'selected' : '' }}>{{ $calendar }}</option>
                        @endforeach
                    </select>
