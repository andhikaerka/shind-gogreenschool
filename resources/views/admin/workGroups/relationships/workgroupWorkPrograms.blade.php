    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table
                    class=" table table-bordered table-striped table-hover datatable datatable-workGroupWorkPrograms">
                    <thead>
                    <tr>
                        <th style="width: 10px;">
                            ...
                            <td>
                                @can('work_program_show')
                                    <a class="btn btn-xs btn-primary"
                                       href="{{ route('admin.work-programs.show', $workProgram->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('work_program_edit')
                                    <a class="btn btn-xs btn-info"
                                       href="{{ route('admin.work-programs.edit', $workProgram->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('work_program_delete')
                                    <form action="{{ route('admin.work-programs.destroy', $workProgram->id) }}"
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            $('.datatable-workGroupWorkPrograms:not(.ajaxTable)').DataTable({buttons: dtButtons})
