...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.work-groups.update", [$workGroup->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('school') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($schools as $id => $school)
                            <option
                                value="{{ $id }}" {{ ($workGroup->school ? $workGroup->school->id : old('school_id')) == $id ? 'selected' : '' }}>{{ $school }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" maxlength="191"
                           name="name"
                           id="name" value="{{ old('name', $workGroup->name) }}" required>
                    @if($errors->has('name'))
