...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.budget-plans.update", [$budgetPlan->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('school') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($schools as $id => $school)
                            <option
                                value="{{ $id }}" {{ ($budgetPlan->school ? $budgetPlan->school->id : old('school_id')) == $id ? 'selected' : '' }}>{{ $school }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('aspect_id') ? 'is-invalid' : '' }}" name="aspect_id" id="aspect_id"
                        ...
                        @foreach(App\BudgetPlan::TOPIC_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('aspect_id', $budgetPlan->aspect_id) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('category') ? 'is-invalid' : '' }}" name="category"
                        ...
                        @foreach($snp_categories as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('category', $budgetPlan->category) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('pic') ? 'is-invalid' : '' }}" type="text"
                           name="pic" id="pic" value="{{ old('pic', $budgetPlan->pic) }}"
                           required>
                    @if($errors->has('pic'))
