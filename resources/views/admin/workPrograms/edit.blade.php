...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.work-programs.update", [$workProgram->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('school') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($schools as $id => $school)
                            <option
                                value="{{ $id }}" {{ old('school_id', ($workProgram->work_group->school_id ?? '')) == $id ? 'selected' : '' }}>{{ $school }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('condition') ? 'is-invalid' : '' }}" name="condition"
                              id="condition" required>{{ old('condition', $workProgram->condition) }}</textarea>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="checkbox" name="featured" id="featured" value="1"
                               {{ $workProgram->featured || old('featured', 0) === 1 ? 'checked' : '' }} required>
...
@section('scripts')
    ...
    <script>
        $(function () {
                ...
                if (selectSchoolId.val()) {
                    ...
                    axios.post('{{ route('api.work-groups') }}', {
                        ...
                        .then(function (response) {
                            ...
                            let workGroupIdVal = Number("{{ old('work_group_id', $workProgram->work_group_id) }}");
            ...
            selectSchoolId.val(("{{ old('school_id', ($workProgram->work_group->school_id ?? '')) }}")).trigger('change');
        });
    </script>
@endsection
