...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.studies.update", [$study->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('quality_report') ? 'is-invalid' : '' }}"
                            name="quality_report_id" id="quality_report_id" required>
                        @foreach($quality_reports as $id => $quality_report)
                            <option
                                value="{{ $id }}" {{ (old('quality_report_id', $study->quality_report->id ?? null)) == $id ? 'selected' : '' }}>{{ $quality_report }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('potential') ? 'is-invalid' : '' }}" name="potential"
                              id="potential" required>{{ old('potential', $study->potential) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('problem') ? 'is-invalid' : '' }}" name="problem"
                              id="problem" required>{{ old('problem', $study->problem) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('activity') ? 'is-invalid' : '' }}" name="activity"
                              id="activity" required>{{ old('activity', $study->activity) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('physical') ? 'is-invalid' : '' }}" name="physical"
                              id="physical" required>{{ old('physical', $study->physical) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('artwork') ? 'is-invalid' : '' }}" name="artwork"
                              id="artwork" required>{{ old('artwork', $study->artwork) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('period') ? 'is-invalid' : '' }}" name="period"
                        ...
                        @foreach(App\Study::PERIOD_SELECT as $key => $label)
                            <option
                                value="{{ $key }}" {{ old('period', $study->period) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
...
@section('scripts')
    <script>
        $(function () {
                ...
                setTimeout(function () {
                    ...
                    axios.post('{{ route('api.quality-reports') }}', {
                        ...
                        .then(function (response) {
                            ...
                            let qualityReportId = Number("{{ old('quality_report_id', $study->quality_report_id ?? null) }}");
                ...
                setTimeout(function () {
                    ...
                    axios.post('{{ route('api.work-groups') }}', {
                        ...
                        .then(function (response) {
                            ...
                            let workGroupIdVal = Number("{{ old('work_group_id', $study->work_group_id ?? null) }}");
                ...
                setTimeout(function () {
                    ...
                    axios.post('{{ route('api.partners') }}', {
                        ...
                        .then(function (response) {
                            ...
                            let partnerIdVal = Number("{{ old('partner_id', $study->partner_id ?? null) }}");
            ...
            selectSchoolId.val(Number('{{ old('quality_report_id', $study->quality_report->school_id ?? null) }}')).trigger('change');
        });
    </script>
@endsection
