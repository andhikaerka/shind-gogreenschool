...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.activity-implementations.store") }}" id="formCreate">
                @csrf
                <div class="form-group">
                    <label class="required"
                           for="school_id">{{ trans('crud.activityImplementation.fields.school') }}</label>
                    ...
                    @endif
                    <span class="help-block">{{ trans('crud.activityImplementation.fields.school_helper') }}</span>
                </div>
                <div class="form-group">
                    ...
                    <input class="form-control date {{ $errors->has('date') ? 'is-invalid' : '' }}"
                           type="text" data-toggle="datetimepicker" data-target="#date"
...
@section('scripts')
            ...
            selectSchoolId.change(function () {
                axios.post('{{ route('api.work-groups') }}', {
                    ...
                    .then(function (response) {
                        ...
                        selectWorkGroupId.trigger('change');
                    });
            });
    ...
    <script>
        ...
        Dropzone.options.documentDropzone = {
            url: '{{ route('admin.activity-implementations.storeMedia') }}',
            maxFilesize: 2, // MB
            acceptedFiles: '.pdf',
            maxFiles: 1,
            addRemoveLinks: true,
